...
namespace Clipper2Lib
{
  ...
  typedef std::pmr::vector<Edge*> VertexEdgeList;

  class Vertex2 {
  public:
    Point64   pt;
    VertexEdgeList edges;
    bool      innerLM = false;
    Vertex2(const Point64& p64, std::pmr::memory_resource* resource)
      : pt(p64), edges(resource) { edges.reserve(2); };
  };
  ...
  class Delaunay {
  private:
    // ImCut patch 0002: all graph objects and per-vertex adjacency buffers have the
    // lifetime of one Execute call. A monotonic call-local arena preserves stable
    // pointers while replacing thousands of individual heap allocations. It is not
    // shared, cached or global, so reentrancy and worker isolation are unchanged.
    std::pmr::monotonic_buffer_resource objectStorage;
    ...
    Vertex2* CreateVertex(const Point64& point);
  ...
  {
    for (auto v : allVertices) std::destroy_at(v);
    allVertices.resize(0);
    for (auto e : allEdges) std::destroy_at(e);
    allEdges.resize(0);
    for (auto t : allTriangles) std::destroy_at(t);
  ...
  Vertex2* Delaunay::CreateVertex(const Point64& point)
  {
    std::pmr::polymorphic_allocator<Vertex2> allocator(&objectStorage);
    Vertex2* result = allocator.allocate(1);
    std::construct_at(result, point, &objectStorage);
    allVertices.push_back(result);
    return result;
  }

  Edge* Delaunay::CreateEdge(Vertex2* v1, Vertex2* v2, EdgeKind k)
  {
    std::pmr::polymorphic_allocator<Edge> allocator(&objectStorage);
    Edge* res = allocator.allocate(1);
    std::construct_at(res);
    allEdges.push_back(res);
    if (v1->pt.y == v2->pt.y)
    {
      res->vB = v1; res->vT = v2;
    }
  ...
  {
    std::pmr::polymorphic_allocator<Triangle> allocator(&objectStorage);
    Triangle* res = allocator.allocate(1);
    std::construct_at(res, e1, e2, e3);
    allTriangles.push_back(res);
  ...
  void Delaunay::MergeDupOrCollinearVertices()
  {
    ...
    //
    // ImCut patch 0001 (see ThirdParty/Clipper2/PATCHES/). Two changes below:
    //
    //   1. The collinear-split loops skip SELF-LOOP edges, vB == vT == v1. Merging two
    //      coincident vertices turns an edge that joined them into a self-loop, and
    //      SplitEdge on a self-loop calls CreateEdge(v1, ...) - which push_backs onto
    //      the very vector being walked. That reallocation is the heap-use-after-free
    //      this patch exists for, and because the new edge also has its bottom at v1 it
    //      re-qualifies for splitting, so the loop feeds itself. A zero-length edge has
    //      no direction, so asking whether it is collinear with anything is meaningless
    //      and skipping it removes work rather than answers.
    //
    //   2. Indices instead of iterators, so a future append cannot resurrect the
    //      dangling-iterator failure silently. With (1) in place nothing appends, and
    //      the two forms are equivalent.
    //
    // The note above this is true of allVertices and false of the per-vertex edge
    // lists, which is what made the original assumption unsafe.
    ...
    for (auto vIter2 = allVertices.begin() + 1; vIter2 != allVertices.end(); ++vIter2)
    {
      ...
      //
      // ImCut patch 0001: skip self-loops, and index rather than iterate.
  ...
  void Delaunay::AddPath(const Path64& path)
  {
    ...
    Vertex2* v0 = CreateVertex(path[i]);
